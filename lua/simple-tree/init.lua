...
local separator = (isWindows == 1 and '\\' or '/')
local file_icons = {
  ["lua"] = "",
  ["json"] = "",
  ['xml']='',
...
local function get_file_icon(is_dir, file)
  ...
  return file_icons[ext] or file_icons["default"]
end
local M = {}
  ...
  if item.is_dir then
    ...
    if item.is_open == false then
      updateValueByIndexPath(files_list_data, pathIndexs, {
        is_open = true,
        name = string.gsub(files_list[line_number].name, folder_icon, folder_open_icon)
      })
      get_current_files(full_path, level)
    else
      updateValueByIndexPath(files_list_data, pathIndexs, {
        is_open = false,
        name = string.gsub(files_list[line_number].name, folder_open_icon, folder_icon)
      })
    end
    render_tree()
  ...
  for i = 1, #pathIndexs do
    ...
    if current ~= nil then
      if current.is_dir == true and current.is_open == false then
        ...
        current.name = string.gsub(current.name, folder_icon, folder_open_icon)
      end
    else
...
local function toggle_tree()
  if is_tree_open then
    ...
    local insert_mode_keys = { 'i', 'a', 'I', 'A', 'o', 'O', 's', 'S', 'X', 'cw', 'C', 'P', '.', 'c', 'x', 'u', 'U', 'd',
      "D", 'p' }
...
function M.setup(user_conf)
  ...
  user_conf = user_conf or { auto_focus_file = true }
